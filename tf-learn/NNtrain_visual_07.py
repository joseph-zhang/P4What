...
with tf.Session() as sess:
    sess.run(init)
    # a merger to merge all summarys
    merger = tf.summary.merge_all()
    # a writer to write something on tensorboard
    ...
    for it in range(1000 + 1):
        ...
        if it % 50 == 0:
            ...
            # write states on tensorboard every 50 times
            rs = sess.run(merger, data_dict)
